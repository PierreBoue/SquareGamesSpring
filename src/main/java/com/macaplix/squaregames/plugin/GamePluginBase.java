...
public abstract class GamePluginBase implements GamePlugin {
    ...
    public GameDescription checkParams(GameParams params) {
        ...
        if (!checkAnswer.isBlank())
            return new GameDescription(params.gameIndex(), "", 0, factory.getGameId(), factory.getGameId(), playerCount, params.boardSize(), new HashMap<CellPosition, Token>(), new ArrayList<>(), new ArrayList<>(), (new Date()).toInstant().getEpochSecond(), 0, "playerCount should be between " + factory.getPlayerCountRange().min() + " and " + factory.getPlayerCountRange().max(), false);
        ...
        if (!checkAnswer.isBlank())
            return new GameDescription(params.gameIndex(), "", 0, "", "", playerCount, boardSize, new HashMap<CellPosition, Token>(), new ArrayList<Token>(), new ArrayList<Token>(), (new Date()).toInstant().getEpochSecond(), 0, "board size should be between " + factory.getBoardSizeRange(params.playerCount()).min() + " and " + factory.getBoardSizeRange(params.playerCount()).max(), false);
